...
├── dist/                    compiled HTML output — gitignored, generated by `npm run build`
└── _partials/
...
```
npm run build                 # compiles both templates to mail-template/dist/
npm run watch:contact         # rebuild on save while editing
npm run watch:contact-reply
```

No local install — the scripts shell out to `npx mjml`, which fetches the
compiler on demand rather than adding a `node_modules/` to the project.

VS Code's MJML extension is configured (`.vscode/settings.json`) to
resolve `mj-include` for live preview.
